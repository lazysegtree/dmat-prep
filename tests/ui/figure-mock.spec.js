...
const bank = JSON.parse(fs.readFileSync(new URL('../../website/data/figure-sequences.json', import.meta.url))).puzzles;
...
for (const [level, mix] of Object.entries(mixes)) {
  test(`${level} mock has exact mix, persists and repeats its level`, async ({ page }) => {
    ...
    expect(() => parseBackup(JSON.stringify({ format: 'dmat-progress', version: 3, progress: { 'latin-squares': [], 'mathematical-equations': [], 'figure-sequences': [saved] } }))).not.toThrow();
...
test('invalid level defaults to Normal and mock expires once after 25 minutes', async ({ page }) => {
  ...
  await page.goto('/figure-sequences/mock/?difficulty=unknown');
  await expect(page.getByLabel('Mock difficulty')).toHaveValue('normal');
