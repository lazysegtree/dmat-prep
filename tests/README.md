...
Shared drill tests (`unit/speed-drill.test.js`, `ui/latin-drill.spec.js`, and `ui/other-drills.spec.js`) cover 5/10-question selection across all three trainers, all nine fixed/mixed distributions, proportional rounding and random tie-breaking, unique puzzles, hidden difficulty during play, custom countdown expiry, stopwatch timing, explicit answer saving, URL normalization, backup restoration, repeat settings, legacy results, and mobile setup layout. Figure drills retain paired scoring and varied shapes. Setup screenshots are saved under `/tmp/dmat-*-drill-*.png`.
