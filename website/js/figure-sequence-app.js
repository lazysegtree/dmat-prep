...
function renderHome() {
 ...
 <a class="mode-card" href="${routeUrl('drill')}"><strong>Speed Drill</strong><span>Choose 5 or 10 sequences, a time limit, and a single or mixed difficulty.</span></a>
...
function renderDrillSetup() {
  renderSharedDrillSetup(app, {
    ...
    instruction: 'Choose both missing frames for each sequence.',
...
function startSession(mode, difficulty = null, selectedQuestions = null, drill = null) {
  ...
  const questions = selectedQuestions || (mode === 'drill'
    ? chooseDrillPuzzles(bank, difficulty, drill.questionCount, { config: STANDARD_DRILL, getDifficulty: question => question.difficulty.level, selectPool: selectDiversePuzzles })
...
function renderQuestion() {
 ...
 app.innerHTML = examMarkup({
  ...
  navigator: examNavigator(session.questions, session.current, (item, index) => answerComplete(item, session.savedAnswers[index]), 'Sequence', session.reviewFlags),
...
function renderResults(result, reviewIndex = null) {
  ...
  app.innerHTML = `
    <section>
      <p class="eyebrow">Figure Sequences · ${MODE_NAMES[result.mode]}${result.mode !== 'learn' ? ` · ${sessionDifficultyName(result)}` : ''} results</p>
      ...
      <p class="lede">${result.automatic ? `Time expired, so the ${result.mode === 'drill' ? 'drill' : 'mock'} was submitted automatically.` : 'Review each figure’s movement, colour, and rotation.'}</p>
...
function showReview(result, index) {
 ...
 detail.innerHTML = `<h2>Sequence ${index + 1}</h2><p class="small muted">Difficulty: ${STANDARD_DRILL.names[question.difficulty.level]}</p>${sequenceMarkup(question, result.answers[index], true)}
